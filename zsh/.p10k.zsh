# Generated by Powerlevel10k configuration wizard on 2023-11-16 at 16:29 SAST.
...
() {
  ...
  typeset -g POWERLEVEL9K_RIGHT_PROMPT_ELEMENTS=(
    ...
    weather                 # example user-defined segment (see prompt_example function below)
  )
    ...
    function prompt_docker() {
        ...
        if [[ "$DOCKER_CONTEXT" != "" ]]; then
            if [[ "$DOCKER_CONTEXT" != "default" ]]; then
                p10k segment -f 255 -i '' -t "$DOCKER_CONTEXT"
            fi
        fi
    }

  function prompt_weather() {
    if [ -f /tmp/weather-line ]; then
        WEATHER=`/bin/cat /tmp/weather-line`
        p10k segment -f 255 -t "$WEATHER"
    fi
  }
